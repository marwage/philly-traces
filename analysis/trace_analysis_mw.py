        ...
        if len(self._attempts) == 0:
            ...
            if self._attempts[0]["start_time"] is None:
                ...
                if self._attempts[-1]["end_time"] is None:
                    ...
                    self._run_time = timedelta_to_minutes(
                        self._attempts[-1]["end_time"] - self._attempts[0]["start_time"]
                    )
                self._queueing_delay = timedelta_to_minutes(
                    self._attempts[0]["start_time"] - self._submitted_time
                )
...
def queuing_delays(jobs):
    ...
    for i, vc in enumerate(vcs):
        for bucket in queueing_delays[vc]:
            ...
            plt.plot(
                x, y, label="%s GPU" % (num_gpus), color=color, linestyle=linestyle
            )
        plt.title("VC %s" % (vc))
        plt.legend(loc="lower right")
        plt.xscale("log")
    ...
    with open(gpu_util_path, "r") as f:
        ...
        for row in reader:
            ...
            gpu_util[machineId][time] = row[
                2:-1
            ]  # Ignore extra empty string at the end
...
def write_to_csv(jobs):
    ...
    df = pd.DataFrame(
        {"id": ids, "num_gpus": ngpus, "runtime": runtimes, "attempts": attempts}
    )
